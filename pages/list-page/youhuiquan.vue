	...
	>
		...
		<view class="inp-block">
			<view class="content-wrap">
				<input class="inp" type="text" v-model="search" placeholder="请输入关键字" />
				<text class="text" @click="searchFn">搜索</text>
			</view>
		</view>
...
export default {
	...
	onLoad(options) {
		...
		that.id = options.id;
		uni.$off('goodsAddCartEd_youhuiquan')
		uni.$on('goodsAddCartEd_youhuiquan', arg => {
	...
	computed: {
		pageBgStyle() {
			...
			let styuleObj = assertObject(goodsListBgStyleObj.yhq);
