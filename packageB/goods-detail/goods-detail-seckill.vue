...
<template>
	<view>
		...
		<seckill-time :end-time="seckillGoodsTime" :goods-info="seckillGoodsInfo" :tip-title="tipTitle"></seckill-time>
			...
			<view class="goods-price-sale">
				<view class="price-box">
					<view class="goods-price-box">
						<view class="icon-price red-price">¥</view>
						<view class="price-txt red-price">{{ detailGoodsShopPrice | filterTransNum }}</view>
						<view class="unit-text">/{{ detailGoodsUnit }}</view>
					</view>
		...
		<add-cart-block
			...
			:pagePath="`packageB/goods-detail/goods-detail-seckill?goods_id=${goodsId}`"
...
import seckillTime from '@/packageB/goods-detail/goods-detail-components/seckill-time-bar.vue'; // 秒杀商品详情 倒计时组件
...
export default {
	...
	onLoad(option) {
		...
		uni.$off('goodsAddCartEd_goodsDetailSeckill');
		uni.$on('goodsAddCartEd_goodsDetailSeckill', arg => {
	...
	methods: {
		...
		setTimeFn() {
			let that = this;
			let miaoshaStartTime = assertString(that.configInfo.rob_start_time) || '8:00';
			let miaoshaEndTime = assertString(that.configInfo.rob_time);
