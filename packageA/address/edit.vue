...
<script>
import { assertArray, assertObject, checkTypeFn } from '@/tools/tools.js';
	...
	methods: {
		...
		getAddressInfoFn(id) {
			...
			return new Promise(async resFn => {
				...
				if (result.statusCode == 200) {
					if (result.data.code == 200) {
						let addressObject = assertObject(result.data.data);
						let [longitude, latitude] = `${addressObject.coordinate}`.split(',');
						resFn({
							type: 'success',
							msg: '获取地址信息成功',
							result: {
								name: addressObject.name,
								sex: addressObject.sex,
								mobile: addressObject.mobile,
								address: addressObject.address,
								housenumber: addressObject.housenumber,
								status: addressObject.status == 1,
								tagId: addressObject.tag,
								longitude,
								latitude
							}
						});
		...
		async loadFn() {
			let that = this;
			let [getAddressTagResult, getAddressInfoResult] = await Promise.all([
				that.getAddressTagFn(),
				that.getAddressInfoFn(that.id)
			]);

			if (getAddressTagResult.type == 'success' && getAddressInfoResult.type == 'success') {
				let {
					name,
					sex,
					mobile,
					...
					longitude,
					latitude
				} = getAddressInfoResult.result;

				let tagList = getAddressTagResult.result;

				that.name = name;

				let sexEnumValue = Object.values(sexEnum).indexOf(sex);
				let sexEnumKeyIndex = sexEnumValue <= 0 ? 0 : sexEnumValue;
				that.sex = Object.keys(sexEnum)[sexEnumKeyIndex];
				that.mobile = mobile;
				that.address = address;
				that.housenumber = housenumber;
				that.defaultStatus = status;
				...
				uni.showToast({
					icon: 'none',
					title: getAddressTagResult.type == 'error' ? getAddressTagResult.msg : getAddressInfoResult.msg
				});
			}
		...
		async saveFn() {
			...
			if (!this.housenumber || this.housenumber.length > 60) {
				uni.showToast({
