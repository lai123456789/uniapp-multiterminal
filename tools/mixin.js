...
let toTopBtnTimeer; // 回到顶部按钮的 定时器


let menuButtonRectInfo = {}; // 小程序胶囊按钮信息
...
let statusBarHeight = getSystemInfoSync.statusBarHeight, // 状态栏 高度
	windowWidth = getSystemInfoSync.windowWidth; // 屏幕宽




let goodsAddCartEdEmitList = ['goodsAddCartEd_gongyingshangSeller', 'goodsAddCartEd_activity',
	'goodsAddCartEd_foodDetail', 'goodsAddCartEd_everydayBuy', 'goodsAddCartEd_huiyuanjieshao',
	'goodsAddCartEd_vipGoodsList', 'goodsAddCartEd_goodsDetailSeckill', 'goodsAddCartEd_goodsDetail',
	'goodsAddCartEd_cart', 'goodsAddCartEd_class', 'goodsAddCartEd_index', 'goodsAddCartEd_caichangIndex',
	'goodsAddCartEd_todaySeckill', 'goodsAddCartEd_homeSwiper', 'goodsAddCartEd_indexHot',
	'goodsAddCartEd_listPageIndex', 'goodsAddCartEd_youhuiquan', 'goodsAddCartEd_searchIndex'
]; // 加入购物车成功后 触发的事件
export default {
	data() {
		return {
			onoff: false,
			WXtop: 0,
			isShowToTopBtn: false, // 是否显示回到顶部按钮
			...
			menuButtonRectInfo,
			mixinScrollTop: 0, // 当前页面滚动的距离
			mixinScrollIsStop: true, // 当前页面 屏幕是否停止滚动

			anniushuzi, // 一个加号按钮按钮右上是数字
			shuanganniushuzi // 一个减号按钮 数字 一个加号按钮按钮
		};
	},
	methods: {
		// 停止3秒钟
		...
		async launchFn(e) {
			...
			// #ifdef H5
			let isWeixn = that.isWeixn();
			that.$store.commit('setBrowserIsWx', that.isWeixn());
			// console.log(that.globalBrowserIsWx);
			// #endif


			that.checkIsiPhoneXFn(); // 查看是否 苹果x

			let sharer = e.query.scene; // 分享人的 加密字符串


			if (sharer && !e.query.shareFn) { // 有加密人字符串 不是通过小程序分享的(海报图片扫码方式)
				that.setSharerStrFn(sharer);
			}

			if (sharer && e.query.shareFn) { // 有加密人字符串 是通过小程序分享的 (转发小程序)
				that.setSharerStrFn_btn(e.query);
			}

			// #ifdef MP-WEIXIN
			let globalSendMsgFlag = await that.getGlobalSendMsgFlag();
			that.$store.commit('setGlobalMainSwitch', globalSendMsgFlag.type == 'success' ? true : false)
			that.$store.commit('setBackIconTop', statusBarHeight + 10);
			// #endif
			...
			that.$store.commit('setAppenv', appPlatformEnum[getSystemInfoSync.platform]);
			// #endif



			uni.$on('goodsAddCartEd', arg => goodsAddCartEdEmitList.forEach(i => uni.$emit(i,
				arg))); // 加入购物车成功后 开始触发页面里事件渲染视图
		},
		...
		*/
		...
		// 小程序静态图片目录
		calcImageSrc(src) {
			return `${this.appStaticPath}${src}`
		},

		/*
			@description 电话号码正则
			@param tel <String> <Number>
			@return <Boolean>
				true 表示验证通过
				false 表示验证没通过
		...
		// 页面跳转函数
		navigateToFn(obj) {
			if (obj.checkLogin && !this.isLogin) { // 需要登录才能跳转
		...
		// ↓↓↓ 给tab-bar角标数字 赋值
		async setTabBarCartNumFn() {
			let that = this;
			if (!that.isLogin) {
				uni.$emit('setCardNumberFn', 0);
				return false;
			}
			let result = await that.getCartNumsumFn();
				...
				if (result.statusCode == 200) {
					if (result.data.code == 200) {
						resultObj = {
							type: 'success',
							msg: result.data.msg,
							result: transNumFn(assertObject(result.data.data).number)
						}
					} else {
						resultObj = {
							type: 'error',
		...
		/* 
			...
			@param
				...
				number <Number>
					...
					减去1个 传-1
		*/
		addShoppingCartFn(goodsId, number, specId) {
			let that = this;
			specId = (() => {
				let n = transNumFn(specId);
				return n <= 0 ? 0 : n;
			})();
				...
				if (result.statusCode == 200) {
					if (result.data.code == 200) {
						resultObj = {
							type: 'success',
							msg: result.data.msg,
							result: transNumFn(assertObject(result.data.data).selectedNum),
						};
		...
		/* 
			...
			@param
				goodsId <String>
					删除 id 是 134传 '134'
					删除 id 是 134和135 传 '134,135' 
					删除多个用逗号分隔
		*/
		delShoppingCartGoodsFn(options) {
			let that = this;
		...
		*/
		changeGoodsSelectStatusFn(id, flag) {
			let that = this;
			return new Promise(async (resFn, rejFn) => {
				let result = await that.$request({
		...
		*/
		...
		/*
			设置分享者加密字符串
			sharer 后台传的编码字符串 'code id 0'
		*/
		setSharerStrFn(sharer) {
			let searchArr = decodeURIComponent(sharer).split(',');
			sharer = searchArr[0];
			console.log(
				'分享人的加密字符串', sharer
			);
		...
		*/
		...
		setConfigFn() {
			...
			return new Promise(async (res) => {
				/* 
				let sleep = await that.sleepFn(10);
				console.log(sleep);
				*/
				let result = await that.$request({
		...
		getTabBar() {
			let that = this,
				...
				configInfo = assertObject(that.configInfo),
				tabBarList = assertArray(configInfo.tabbar),
				menuList = tabBarList.map((tatBarItem, index) => {
					let typeObj = {};
					// 判断是购物车 加上标识符
		...
		*/
		...
		async hasTokenGetUserInfo(fn) {
			...
			console.log(
				'个人中心有登录状态下返回的两个数据', [myPageResult, userInfoResult],
			);
		...
		*/
		async getSpecInfoFn(goodsId) {
			...
			return new Promise(async (resFn) => {
				...
				if (result.statusCode == 200) {
					if (result.data.code == 200) {
						let resObj = assertObject(result.data.data);

						let goodsSpecRowsList = assertObject(resObj.spec_goods_price); // 多规格项


						let goodsSpecSelectItemList = addSpecFilterItemActAttrFn(assertArray(resObj
							.filter_spec)); // 多规格 笛卡尔积项对应的 商品 信息
	...
	computed: {
		...
		*/
		isLogin() {
			return assertString(this.globalToken).length > 0;
		},
		/*
		...
		*/
		...
		calcOpenIngPayData() {
			let that = this,
				configInfo = assertObject(that.configInfo),
				paysConfig = assertObject(configInfo.pays_config);
			return paysConfig;
		},
		// 返回微信支付下开通的平台
		calcOpenIngWeiXinPayTypeArr() {
			let that = this,
				payTypeObject = assertObject(that.calcOpenIngPayData.wechat),
				payTypeArr = Object.keys(payTypeObject);
			return payTypeArr.filter(i => payTypeObject[i] == 1);
		},
		// 返回支付宝支付下开通的平台
		calcOpenIngZhifubaoPayTypeArr() {
			let that = this,
				payTypeObject = assertObject(that.calcOpenIngPayData.alipay),
				payTypeArr = Object.keys(payTypeObject);
			return payTypeArr.filter(i => payTypeObject[i] == 1);
		},
		// 安卓app平台是否支持微信支付
		calcWeixinPayAndroidApp() {
			return assertArray(this.calcOpenIngWeiXinPayTypeArr).includes(openIngEnumObj[openIngApp]);
		},
		// h5平台是否支持微信支付
		calcWeixinPayH5() {
			return assertArray(this.calcOpenIngWeiXinPayTypeArr).includes(openIngEnumObj[openIngH5]);
		},
		// 微信小程序平台是否支持微信支付
		calcWeixinPayMiniprogram() {
			return assertArray(this.calcOpenIngWeiXinPayTypeArr).includes(openIngEnumObj[openIngWxminiprogram]);
		},
		// 公众号平台是否支持微信支付
		calcWeixinofficialaccount() {
			return assertArray(this.calcOpenIngWeiXinPayTypeArr).includes(openIngEnumObj[openIngWxofficialaccount]);
		},

		// 安卓app平台是否支持支付宝支付
		calcZhifubaoPayAndroidApp() {
			return assertArray(this.calcOpenIngZhifubaoPayTypeArr).includes(openIngEnumObj[openIngApp]);
		},
		// h5平台是否支持支付宝支付
		calcZhifubaoPayH5() {
			return assertArray(this.calcOpenIngZhifubaoPayTypeArr).includes(openIngEnumObj[openIngH5]);
		},
		// 微信小程序平台是否支持支付宝支付
		calcZhifubaoPayMiniprogram() {
			return assertArray(this.calcOpenIngZhifubaoPayTypeArr).includes(openIngEnumObj[openIngWxminiprogram]);
		},
		// 公众号平台是否支持支付宝支付
		calcZhifubaoofficialaccount() {
			return assertArray(this.calcOpenIngZhifubaoPayTypeArr).includes(openIngEnumObj[openIngWxofficialaccount]);
		},
		...
		...mapState({
			...
			// #ifdef H5
			globalBrowserIsWx: store => store.browserIsWx, // 当前环境是否是基于公众号微信浏览器
			// #endif

			// #ifdef APP-PLUS
			appenv: store => store.appenv, // 当前运行环境 安卓 or 苹果
			// #endif
			goodsAddCartStyle: store => {
				let goodsAddCartStyleFlag = transNumFn(assertObject(store.configInfo).spjganys);
				if (goodsAddCartStyleFlag == 1) {
					return shuanganniushuzi;
				}
				return anniushuzi;
			}, // 商品右下角的加入购物车按钮的样式
			/*
			...
			*/
			gongyingshang: store => {
				return transNumFn(assertObject(store.configInfo).system_pattern) == 2
			},


			

			backIconTop: store => store.backIconTop, // 详情页返回上一页的左上角图标距离顶部的位置
			/*
			...
			*/
			...
			/*
				是否在提交苹果审核
					...
					false	不在审核
			*/
			appleCheckOpen(store) {
				return transNumFn(assertObject(store.configInfo).is_iphone_audit) == 1
			},
		}),
		...mapGetters({
			getIsOpenEat: 'getIsOpenEat', // 是否开启吃了吗
			isOpenVipFunc: 'isOpenVipFunc', // 是否开启了会员功能
			miniProgrameCheck: 'miniProgrameCheck', // 小程序是否在审核
	...
	onPageScroll(e) {
		...
		toTopBtnTimeer = setTimeout(() => {
			if (e.scrollTop > 600) {
				this.isShowToTopBtn = true;
			} else {
				this.isShowToTopBtn = false;
			}
			this.mixinScrollIsStop = true;
			// console.log('this.mixinScrollIsStop',this.mixinScrollIsStop);
		}, 100)
	},
